...
The behavior of {% raw %}`{{greeting}}`{% endraw %} in the HTML file is controlled by `Template` within the client-side code in the JS file, as well as the handling of events.
